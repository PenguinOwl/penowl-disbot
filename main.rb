...
def pres(mem)
  begin
    return Math.log10(((conn.mget(mem, :bal).to_i)-(conn.mget(mem,:tax).to_i)).to_f).to_i - 7
...
def command(command,event,args)
  if ENV['DEBUG'] == 'true'
    unless conn.mget(event.author, :state) == "1" and not command == "unfreeze"
    ...
    begin
      begin
        ...
        mem = event.author
        if conn.mget(mem, :month) != (Date.today.year.to_s + "-" + Date.today.month.to_s)  && conn.mget(mem, :state) == "0"
          conn.mset(event.author, :tax, conn.mget(event.author, :tax).to_i+conn.mget(event.author, :taxamt).to_i)
        end
        event.respond("Argument error!")
      end
    rescue NoMethodError
      mem = event.author
      if conn.mget(mem, :month) != (Date.today.year.to_s + "-" + Date.today.month.to_s)
        conn.mset(event.author, :tax, conn.mget(event.author, :tax).to_i+conn.mget(event.author, :taxamt).to_i)
      end
...
def tax(dfs, event)
  begin
    ...
    if conn.mget(mem, :month) != (Date.today.year.to_s + "-" + Date.today.month.to_s)
      conn.mset(mem, :tax, conn.mget(event.author, :tax).to_i+conn.mget(mem, :taxamt).to_i)
    end
  rescue NoMethodError
  end
end
...
class Command
  ...
  def Command.taxes(event, *args)
    ...
    if event.message.mentions.size == 0
      if conn.mget(mem, :month) != (Date.today.year.to_s + "-" + Date.today.month.to_s)
        event.respond("You owe $#{conn.mget(mem, :tax).mon} to the IRS. You have $#{conn.mget(mem, :bal).mon}.")
      else
        event.respond("You have paid your taxes.")
      end
    end
    event.message.mentions.each do |mem|
      if conn.mget(mem.on(event.channel.server), :month) != (Date.today.year.to_s + "-" + Date.today.month.to_s)
        mem = mem.on(event.channel.server)
        event.respond(mem.mention + " owes $#{conn.mget(mem, :tax).mon} to the IRS. They have $#{conn.mget(mem, :bal).mon}.")
      else
  ...
  def Command.prestige(event, arg="elolo")
    ...
    if pres(mem) > 0
      if arg == "confirm"
        mem = event.author
        conn.pset(mem, :lvl, conn.pget(mem, :lvl).to_i + pres(mem))
        conn.pset(mem, :points, conn.pget(mem, :points).to_i + pres(mem))
  ...
  def Command.upgrade(event, type="info")
    if ["bonus", "steal", "auto"].include? type
      ...
      if conn.pget(mem, :points).to_i>=(conn.pget(mem, type.to_sym).to_i+1)
        conn.pset(mem, :points, conn.pget(mem, :points).to_i - (conn.pget(mem, type.to_sym).to_i+1))
        conn.pset(mem, type.to_sym, (conn.pget(mem, type.to_sym).to_i+1))
        event.respond "**Upgraded your** `#{type}` **skill to level #{conn.pget(mem, type.to_sym)}.**"
      else
  ...
  def Command.money(event, *args)
    ...
    if event.message.mentions.size == 0
      ...
      event.respond("**You have $#{conn.mget(mem, :bal).mon}.**")
    end
    event.message.mentions.each do |mem|
      mem = mem.on(event.channel.server)
      event.respond("**" + mem.mention + " has $#{conn.mget(mem, :bal).mon}.**")
    end
  end
  ...
  def Command.id(event, *args)
    ...
    if event.message.mentions.size == 0
      ...
      event.respond("**You are id $#{conn.mget(mem, :id)}.**")
    end
    event.message.mentions.each do |mem|
      mem = mem.on(event.channel.server)
      event.respond("**" + mem.mention + " is id $#{conn.mget(mem, :id)}.**")
    end
  end
  ...
  def Command.info(event, *args)
    ...
    if event.message.mentions.size == 0
      ...
      conc = n + "'s Stats\n$$\nTax: #{s}\nBalance: $#{conn.mget(mem, :bal).mon}\nHourly Reward: $#{conn.mget(mem, :daily).mon}\nTax Rate: $#{conn.mget(mem, :taxamt).mon} per message\nInvestments: #{conn.mget(mem, :invest).to_s}\nInvestment Cost: $#{conn.mget(mem, :invcost).mon}\nTimes Lobbied: #{conn.mget(mem, :lbcount)}"
      pb = conn.pget(mem, :lvl).to_i
      if pb > 0
        conc << "\n$$\nPrestige\n$$\nLevel: #{conn.pget(mem, :lvl)}\nPoints available: #{conn.pget(mem, :points)}\nStealer level: #{conn.pget(mem, :steal)}\nBonus level: #{conn.pget(mem, :bonus)}\nAuto level: #{conn.pget(mem, :auto)}"
      end
    end
    event.message.mentions.each do |mem|
      s = ""
  ...
  def Command.reward(event)
    mem = event.author
    unless conn.mget(mem, :day) == todays
      conn.mset(mem, :bal, conn.mget(mem, :daily).to_i + conn.mget(mem, :bal).to_i)
      conn.mset(mem, :day, todays)
      event.respond "**Collected $#{conn.mget(mem, :daily).mon} from the bank.**"
      pb = conn.pget(mem, :bonus).to_i
      if pb > 0
        bonus = 0.1 * pb
        conn.mset(mem, :bal, (conn.mget(mem, :daily).to_f.*bonus).to_i + conn.mget(mem, :bal).to_i)
        per = (bonus*100).to_i
  ...
  def Command.lobby(event, type)
    mem = event.author
    unless conn.mget(mem, :lbday) == Date.today.to_s
      if ["taxes", "investments", "rates"].include? type
        conn.mset(mem, :lbday, Date.today.to_s)
        conn.mset(mem, :lbcount, conn.mget(mem, :lbcount).to_i + 1)
        chance = 4
        ...
        if rand(chance) == 1
          ...
          conn.mset(mem, :tax, conn.mget(mem, :lbcount).to_i*conn.mget(mem, :invcost).to_i/2 + conn.mget(mem, :tax).to_i)
        else
          if type == "rates"
            event.respond "**You successfully lobbied the tax code!**"
            unless conn.mget(mem, :taxamt).to_i < 4
              conn.mset(mem, :taxamt, conn.mget(mem, :taxamt).to_i*(60+rand(30))/100)
            end
          elsif type == "investments"
            event.respond "**You successfully lobbied the stock market!**"
            conn.mset(mem, :invcost, conn.mget(mem, :invcost).to_i - (conn.mget(mem, :invcost).to_i / 3))
          elsif type == "taxes"
            event.respond "**You successfully lobbied your local tax collecter!**"
            conn.mset(mem, :tax, conn.mget(mem, :tax).to_i / 2) 
          end
        end
      else
  ...
  def Command.pay(event, ment, amt)
    ...
    bal = conn.mget(mem, :bal).to_i
    if conn.mget(mem, :payc).to_i < (Time.now-120).to_i
      if conn.mget(event.message.mentions[0].on(event.channel.server), :invcost).to_i*2 > amt
        if amt <= conn.mget(mem, :bal).to_i
          if event.message.mentions.size == 1
            mem2 = event.message.mentions[0].on(event.channel.server)
            conn.mset(mem, :bal, bal-amt)
            conn.mset(mem2, :bal, conn.mget(mem2, :bal).to_i + amt)
            event.respond "**Paid " + mem2.mention + " $#{sprintf "%.2f", amt.to_f * 0.01}.**"
            conn.mset(mem, :payc, (Time.now).to_i)
          else
  ...
  def Command.invest(event)
    mem = event.author.on(event.channel.server)
    if conn.mget(mem, :invcost).to_i <= conn.mget(mem, :bal).to_i
      event.respond "*Invested $#{sprintf "%.2f", conn.mget(mem, :invcost).to_f * 0.01} into the stock market.*"
      conn.mset(mem, :bal, conn.mget(mem, :bal).to_i - conn.mget(mem, :invcost).to_i)
      diff = (rand(conn.mget(mem, :invcost).to_i/5) - conn.mget(mem, :invcost).to_i/20) * 2
      if diff > 0
      ...
      conn.mset(mem, :invcost, conn.mget(mem, :invcost).to_i + (conn.mget(mem, :invcost).to_i / 5) + diff)
      conn.mset(mem, :daily, conn.mget(mem, :daily).to_i + diff)
      conn.mset(mem, :invest, conn.mget(mem, :invest).to_i + 1)
      irs = rand(2)
      if irs == 1 and diff > 0 
        event.respond "*The IRS saw your investment and decided to raise your taxes.*"
        conn.mset(mem, :taxamt, conn.mget(mem, :taxamt).to_i + conn.mget(mem, :invcost).to_i / 100)
      else
        irs = 0
      end
    else
  ...
  def Command.paytaxes(event)
    mem = event.author
    if conn.mget(mem, :month) != (Date.today.year.to_s + "-" + Date.today.month.to_s)
      if conn.mget(mem, :tax).to_i <= conn.mget(mem, :bal).to_i
        if taxdays(Date.today)
          conn.mset(mem, :bal, conn.mget(mem, :bal).to_i - conn.mget(mem, :tax).to_i)
          conn.mset(mem, :tax, 0)
          conn.mset(mem, :month, Date.today.year.to_s + "-" + Date.today.month.to_s)
